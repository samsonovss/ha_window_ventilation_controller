...
- окно уже открыто не меньше, чем `Fan Минимальная позиция окна`
...
- температура не снизилась достаточно за `Fan Таймаут без охлаждения`
...
Ручное управление имеет приоритет в `Авто`: если включить или выключить вытяжку руками через её сущность, интеграция удерживает это состояние на время `Fan Таймаут ручного управления`, а потом возвращает управление автоматике.
...
Отдельный `Fan статус` объясняет логику вытяжки:
...
- `Fan режим`: `Выключено`, `Авто`, появляется только если выбрана вытяжка `fan` или `switch`
...
- настройки Fan: минимальная позиция окна, таймаут без охлаждения, минимальное снижение температуры, максимум работы, пауза и таймаут ручного управления
